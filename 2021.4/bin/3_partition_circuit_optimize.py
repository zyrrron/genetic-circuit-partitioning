		...
		if target_n == ['']:
			...
			for npart in sorted(nparts):
				...
				if len(list(G.nodes()))>=20:
					...
					gp.optimize_signal_subnetwork_qs (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, 3, low_constraint, loop_free, priority, trajectories, outdir + str(npart) +'/optimized_lc/')
